...
folder are mapped to the application's root namespace, 'PostgrestTest'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "PostgrestTest"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'PostgrestTest' in the same way
as `"PostgrestTest/sass/src"`.
